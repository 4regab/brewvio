...
// Returned on successful login: the JWT plus basic identity for the SPA.
public record LoginResponse(string Token, string Username, string FullName, string Role);
// Current authenticated user's identity, returned by the "me" endpoint.
...
// Result of a sign-up: the new account's id, username, and initial status.
