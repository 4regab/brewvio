...
const Auth = (() => {
  ...
  function loginScreen(role = 'Cashier') {
    ...
    // Disable Sign In until both username and password are filled.
    const syncSubmit = () => { submit.disabled = !user.value.trim() || !pass.value; };
    user.addEventListener('input', syncSubmit);
    pass.addEventListener('input', syncSubmit);
    syncSubmit();
