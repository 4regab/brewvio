...
(function () {
  ...
  // Period presets drive both the date range and the trend bucketing the API uses.
  const PERIODS = {
    ...
    render: async (root) => {
      ...
      const load = async () => {
        ...
        try {
          ...
          results.appendChild(el('div', { class: 'row g-3 mt-1' },
            el('div', { class: 'col-lg-8' }, el('div', { class: 'section-card p-3 h-100' },
              el('h3', { class: 'h6 mb-3', text: `Revenue Trend — ${PERIODS[period].label}` }),
              el('div', { style: 'position:relative;height:300px' }, trendCanvas))),
            el('div', { class: 'col-lg-4' }, el('div', { class: 'section-card p-3 h-100' },
              el('h3', { class: 'h6 mb-3', text: 'Sales by Category' }),
              el('div', { style: 'position:relative;height:300px' }, catCanvas)))));
          ...
          results.appendChild(el('div', { class: 'section-card p-0 mt-3' },
            el('div', { class: 'p-3 border-bottom' }, el('h3', { class: 'h6 mb-0', text: 'Menu Performance & Profitability' })),
      ...
      const head = el('div', { class: 'd-flex flex-wrap gap-3 align-items-end' },
        ...
        el('div', { class: 'd-flex gap-2 ms-auto' },
          button('<i class="bi bi-filetype-csv"></i> CSV', 'btn-outline-secondary', () => Api.download('/api/reports/export/csv' + qs(), 'sales.csv').catch((e) => toast(e.message, 'danger'))),
          button('<i class="bi bi-filetype-pdf"></i> PDF', 'btn-outline-secondary', () => Api.download('/api/reports/export/pdf' + qs(), 'sales.pdf').catch((e) => toast(e.message, 'danger')))));

      root.innerHTML = '';
      root.appendChild(el('div', { class: 'section-card p-3' }, head));
    ...
    render: async (root) => {
      ...
      const load = async () => {
        ...
        try {
          ...
          // Profitability table sorted by profit.
      ...
      const head = el('div', { class: 'd-flex flex-wrap gap-3 align-items-center' },
        el('h3', { class: 'h5 mb-0 me-auto', text: 'Menu Performance' }),
        segControl(period, pickPeriod));
