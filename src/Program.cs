...
// Build the host: load configuration and register services, then configure the request pipeline below.
...
// Register MVC controllers and the EF Core (Npgsql) DbContext.
...
// Configure JWT bearer authentication to validate the app's own HMAC-signed tokens.
builder.Services
...
// Build the app and configure the middleware pipeline below (ordering matters).
...
// Serve the SPA's static files, then authenticate the request before the revocation check and authorization below.
app.UseDefaultFiles();
app.UseStaticFiles();
...
// Enforce authorization policies, then route requests to the controllers.
...
// Start handling requests (Kestrel locally; the Lambda runtime when hosted on AWS).
await app.RunAsync();
