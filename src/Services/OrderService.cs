...
{
    // Prices and places an order: builds line items, applies discount and VAT-inclusive tax,
    // validates payment, deducts recipe ingredients, records the sale, and returns the receipt.
    // req: the cart items, discount, and payment(s).
    // ct: cancellation token.
    // returns: the receipt for the placed order.
    // throws ArgumentException: when the cart is empty or the payment is insufficient.
    ...
    // Refunds a completed transaction: marks it Refunded, restores ingredient stock, and audits it.
    // id: the transaction id to refund.
    // reason: the required refund reason.
    // ct: cancellation token.
    // returns: the receipt for the refunded transaction, or null if not found.
    // throws ArgumentException: when no reason is given.
    // throws InvalidOperationException: when the transaction is not Completed.
    ...
    // Lists the current cashier's saved drafts (newest first).
    // ct: cancellation token.
    // returns: the cashier's drafts as DTOs.
    ...
    // Deletes one of the current cashier's drafts (no-op if not found) and audits it.
    // id: the draft transaction id to delete.
    // ct: cancellation token.
    ...
    // Maps a draft transaction to its DTO, including an item summary string.
    // tx: the draft transaction to map.
    // returns: the draft DTO.
    ...
    // Advances an order to its next queue status (Preparing -> Completed) and audits the move.
    // id: the transaction id to advance.
    // ct: cancellation token.
    // returns: the updated transaction summary, or null if not found.
    // throws InvalidOperationException: when the order can't be advanced from its current status.
    ...
    // Counts orders still in the active queue (Pending or Preparing).
    // ct: cancellation token.
    ...
    // Computes the next order number (current max transaction id + 1).
    // ct: cancellation token.
    ...
    // Lists recent transactions (newest first) within an optional date range.
    // take: maximum number of transactions to return.
    ...
    // ct: cancellation token.
    // returns: the recent transactions as summary DTOs.
    ...
    // Gets the receipt for a transaction by id.
    // id: the transaction id.
    // ct: cancellation token.
    // returns: the receipt DTO, or null if not found.
    ...
    // Maps a transaction to a receipt DTO, including line items, payments, and warnings.
    // tx: the transaction to map.
    // cashier: the cashier display name shown on the receipt.
    // tendered: the amount tendered by the customer.
    // change: the change due to the customer.
    // warnings: low-stock warnings to surface on the receipt.
    // returns: the receipt DTO.
