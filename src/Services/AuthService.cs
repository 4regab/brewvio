...
// Authentication engine — login with JWT issuance, self-service registration, and account-status checks.
public class AuthService(BrewvioDbContext db, IConfiguration config, AuditService audit)
{
    ...
    // Builds and signs a 2-hour JWT bearer token carrying the user's id, name, full name, and role.
    // user: the authenticated user the token represents.
    // issuedAt: the issued-at timestamp embedded in the token (used for later revocation checks).
    // returns: the serialized signed JWT string.
    // throws InvalidOperationException: when no JWT signing key is configured.
