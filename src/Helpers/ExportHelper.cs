...
{
    // Registers the QuestPDF Community license once per process (required before generating any PDF).
    static ExportHelper() => QuestPDF.Settings.License = LicenseType.Community;

    // ---------- CSV ----------
    // Escapes a value for CSV: wraps it in quotes and doubles embedded quotes when it contains a comma, quote, or newline.
    // v: the value to format (null becomes empty string)
    // returns: a CSV-safe string representation
    ...
    // Encodes a string as UTF-8 with a leading byte-order mark.
    // s: the text to encode
    // returns: UTF-8 bytes prefixed with a BOM
    private static byte[] Utf8(string s) => new UTF8Encoding(true).GetBytes(s); // BOM => opens cleanly in Excel

    // Builds the sales report CSV: summary metrics, revenue trend, and menu performance sections.
    // r: the report data to render
    // returns: UTF-8 CSV bytes
    ...
    // Builds the inventory CSV with one row per ingredient.
    // items: the ingredients to export
    // returns: UTF-8 CSV bytes
    ...
    // Builds the stock-movement ledger CSV with one row per movement.
    // rows: the stock movements to export
    // returns: UTF-8 CSV bytes
    ...
    // Builds an XLSX workbook listing orders, one row per transaction summary.
    // orders: the order summaries to export
    ...
    // Builds an XLSX workbook with three sheets: Summary, Revenue Trend, and Menu Performance.
    // r: the report data to render
    // fromUtc: inclusive start of the reporting period
    // toUtc: exclusive end of the reporting period (shown as the inclusive last day)
    // returns: the .xlsx file bytes
    ...
    // ---------- PDF ----------
    // Builds the sales report PDF: header, summary table, and menu performance table.
    // r: the report data to render
    // storeName: store name shown in the header
    // currency: currency prefix applied to money values
    // fromUtc: inclusive start of the reporting period
    // toUtc: exclusive end of the reporting period (shown as the inclusive last day)
        ...
        Document.Create(doc => doc.Page(page =>
        {
            ...
            page.Content().PaddingVertical(15).Column(col =>
            {
                ...
                col.Item().Table(t =>
                {
                    ...
                    // Adds a key/value row to the summary table (key left-aligned, value right-aligned).
                    // k: the metric label
                    // v: the metric value text
