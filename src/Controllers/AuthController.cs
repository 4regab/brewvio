...
public class AuthController(AuthService auth) : ControllerBase
{
    ...
    // Records a hit for the given bucket+IP and reports whether the IP has exceeded the limit this window.
    // bucket: logical action name used to namespace the counter (e.g. "login")
    // limit: max allowed hits per IP within the window
    // returns: true if the caller is over the limit and should be throttled
    ...
    // Authenticates a user, applying the per-IP rate limit before hitting the auth service.
    // req: username and password credentials
    // ct: cancellation token
    // returns: 200 OK with the auth response on success, 401 on bad credentials, 429 when rate limited
    [AllowAnonymous]
    [HttpPost("login")]
